...
rfile = '../../HaaLimits/python/datacards_shape/MuMuTauTau/mmmt_mm_{}_parametric_unbinned_unblind_{}_TauMuTauMu_{}_MVAMedium_DG_wFakeTauScaleFit_PPonly.root'.format(yvar,massRange,year)
tfile = ROOT.TFile.Open(rfile)
print rfile

ws = tfile.Get('w')
#ws.Print()
pdf_x = ws.pdf('bg_TauMuTauMu_{}_PP_x'.format(year))
pdf_x_cont = ws.pdf('cont_TauMuTauMu_{}_PP_x'.format(year))
if massRange == 'lowmass':
    pdf_x_res = ws.pdf('jpsi_TauMuTauMu_{}_PP_x'.format(year))
elif massRange == 'upsilon':
    pdf_x_res = ws.pdf('upsilon_TauMuTauMu_{}_PP_x'.format(year))
else:
    pdf_x_res = None
pdf_y = ws.pdf('bg_TauMuTauMu_{}_PP_y'.format(year))
...
data = ws.data('data_obs_TauMuTauMu_{}_PP'.format(year))
...
sig_x = ws.pdf('ggH_haa_{}_TauMuTauMu_{}_PP_x'.format(h,year))
sig_y = ws.pdf('ggH_haa_{}_TauMuTauMu_{}_PP_y'.format(h,year))
...
for ext in ['png']:
    canvas.Print('postfit_mm_TauMuTauMu_{}_{}.{}'.format(massRange,year,ext))
...
pull.GetXaxis().SetLabelSize(0.12)
pull.GetXaxis().SetTitleOffset(1)
pull.GetXaxis().SetTitle('m(#mu#mu#mu#mu) (GeV)')
pull.GetXaxis().SetNdivisions(510)
pull.GetXaxis().SetTickLength(0.09)
...
for ext in ['png']:
    ...
    canvas.Print('postfit_{}_TauMuTauMu_{}_{}.{}'.format(yvar,massRange,year,ext))
