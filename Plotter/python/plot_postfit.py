...
rfile = '../../HaaLimits/python/datacards_shape/MuMuTauTau/mmmt_mm_{}_parametric_unbinned_{}_TauMuTauHad_V2_2018_MVAMedium_DG_DoubleExpo_yRange_wFakeTauScaleFit_PPonly.root'.format(yvar,massRange)
...
ws = tfile.Get('w')
#ws.Print()
pdf_x = ws.pdf('bg_TauMuTauHad_V2_2018_PP_x')
pdf_x_cont = ws.pdf('cont_TauMuTauHad_V2_2018_PP_x')
if massRange == 'lowmass':
    pdf_x_res = ws.pdf('jpsi_TauMuTauHad_V2_2018_PP_x')
elif massRange == 'upsilon':
    pdf_x_res = ws.pdf('upsilon_TauMuTauHad_V2_2018_PP_x')
else:
    pdf_x_res = None
pdf_y = ws.pdf('bg_TauMuTauHad_V2_2018_PP_y')


pdf_control = ws.pdf("bg_control_2018")
pdf_control_cont = ws.pdf("contpoly_control_2018")
if massRange == 'lowmass':
    pdf_control_res = ws.pdf("jpsi_control_2018")
elif massRange == 'upsilon':
    pdf_control_res = ws.pdf("upsilon_control_2018")
else:
    pdf_control_res = None


fpostfit = open('../../HaaLimits/python/Impacts_TauMuTauHad_{}_2018/fit.log'.format(massRange))
params={}
...
data = ws.data('data_obs_TauMuTauHad_V2_2018_PP')
...
sig_x = ws.pdf('ggH_haa_{}_TauMuTauHad_V2_2018_PP_x'.format(h))
sig_y = ws.pdf('ggH_haa_{}_TauMuTauHad_V2_2018_PP_y'.format(h))
...
integral_control_cont = params['integral_contpoly_control_2018']
if massRange == 'lowmass':
    integral_control_res = params['integral_jpsi1S_control_2018']+params['relNorm_jpsi2S_2018']*params['integral_jpsi1S_control_2018']
elif massRange == 'upsilon':
    integral_control_res = params['integral_upsilon1S_control_2018']+params['relNorm_upsilon2S_2018']*params['integral_upsilon1S_control_2018']+params['relNorm_upsilon3S_2018']*params['integral_upsilon1S_control_2018']
else:
    integral_control_res = 0
integral_control = integral_control_cont+integral_control_res


sigintegral = ws.function('fullIntegral_ggH_haa_{}_TauMuTauHad_V2_2018_PP'.format(h)).getVal() * br/0.001
#sigintegral = -13.2224
#sigintegral = 0
#mh.setVal(250)
...
################################ control ######################################
x = ws.var('invMassMuMu_control_2018')
print x.getBins()
...
newFrame = newVar.frame(ROOT.RooFit.Range(*newRange),ROOT.RooFit.Binning(int((xRange[1]-xRange[0])/bWidth)))


pdf_control.plotOn(newFrame,ROOT.RooFit.Range(*newRange),ROOT.RooFit.Normalization(data_control.sumEntries("invMassMuMu_control_2018>{} && invMassMuMu_control_2018<{}".format(*newRange))),ROOT.RooFit.Binning(int((xRange[1]-xRange[0])/bWidth)))
data_control.plotOn(newFrame,ROOT.RooFit.Binning(int((xRange[1]-xRange[0])/bWidth)),ROOT.RooFit.MarkerSize(0.01))
...
if massRange == 'lowmass' or massRange == 'upsilon':
    ...
    #newFrame.SetMaximum(xmax*100)
    #newFrame.SetMinimum(1000)
...
for ext in ['png']:
    canvas.Print('postfit_mm_control_{}.{}'.format(massRange,ext))
...
for ext in ['png']:
    canvas.Print('postfit_mm_TauMuTauHad_{}.{}'.format(massRange,ext))
...
for ext in ['png']:
    ...
    canvas.Print('postfit_{}_TauMuTauHad_{}.{}'.format(yvar,massRange,ext))
